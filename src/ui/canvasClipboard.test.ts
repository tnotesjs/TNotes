...
describe('pasteCanvasOutline', () => {
  it('把 Markdown 列表作为当前主题后的同级子树插入', () => {
    ...
    expect(session.document.root.children.map((node) => node.content.text)).toEqual(['a', 'x', 'y', 'b'])
  ...
  it('普通多行文字会转换为同级主题', () => {
    ...
    expect(session.document.root.children.map((node) => node.content.text)).toEqual(['a', '第一行', '第二行'])
  })
})
