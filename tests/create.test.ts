...
describe("createKnowledgeBase", () => {
  it("writes the minimal layout with a starter note", async () => {
    ...
    const config = JSON.parse(
      await fs.readFile(path.join(result.rootPath, "tnotes.json"), "utf8"),
    );
    ...
    const note = await fs.readFile(
      path.join(result.rootPath, result.starterNoteRelPath),
      "utf8",
    );
    expect(note).toMatch(/^---\nid: .+\n---\n/);
  ...
  it("scaffolds package.json, pages workflow, readme, and git init when requested", async () => {
    ...
    expect(await fs.readFile(path.join(result.rootPath, "README.md"), "utf8")).toContain(
      "# 完整库",
    );
  ...
  it("defaults title to folderName and rejects collisions", async () => {
    ...
    await expect(
      createKnowledgeBase({ parentDir: parent, folderName: "alone" }),
    ).rejects.toThrow(/已存在/);
