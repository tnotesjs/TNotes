  ...
  const walk = (nodes: TocNode[], indent: number): void => {
    for (const node of nodes) {
      if (node.kind === "group") {
        ...
        lines.push(
          buildNoteLine(node.index, noteTitles.get(node.index) ?? "", node.done, indent),
        );
      }
...
export function moveSubtree(
  ...
  const targetIndent =
    placement === "inside" ? target.indentLevel + 1 : target.indentLevel;
...
export function setNoteDoneLine(
  ...
  const parsed = parseTocLine(lines[lineIndex]);
  const result = [...lines];
  result[lineIndex] = buildNoteLine(
    parsed.noteIndex!,
    parsed.title ?? "",
    done,
    parsed.indentLevel,
...
export function setNoteTitleLine(
  lines: string[],
  noteIndex: string,
  title: string,
): string[] {
