...
- 代码块收起 / 展开（Desk 与站点同一套规则，规则在 `@tnotesjs/ui/code` 的 `collapse.ts`）：**纯视图状态**，不写回 markdown、不写 localStorage，重开笔记一律展开；只有长代码块（> 20 行）标题左侧才有折叠 Icon（编辑器里是 `.tools` 里的 `.desk-code-collapse`，`order: -1` 固定在标题左侧）；光标点进收起的块会自动展开；代码分组的 Icon 在 tab 行最左侧，切换 tab 自动展开新面板（`deskRawBlockView/container.ts` 的 `activateTab`）。站点侧由 `CodeBlock.vue`/`CodeGroup.vue` SSR 出 Icon，`hydrateIslands.ts` 用同一套 class 契约补行为。
