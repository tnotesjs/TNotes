...
export function pasteCanvasOutline(
  ...
  session.transact((doc) => {
    const parent =
      anchor === session.focusRootNode ? anchor : (anchor.parent ?? session.focusRootNode)
    let index =
      anchor === session.focusRootNode
  ...
  session.selectMany(
    insertedIds,
    insertedIds[insertedIds.length - 1] ?? null,
    insertedIds[0] ?? null
  )
  return insertedIds
}
