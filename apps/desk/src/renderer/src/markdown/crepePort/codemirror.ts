/**
 * 移植自 `@milkdown/crepe@7.22.1` 的 `lib/esm/feature/code-mirror`（MIT）。
 *
 * 改动：去掉 Crepe 的 `FeaturesCtx`/`CrepeCtx`；其余（CodeMirror 扩展开关、
 * basicSetup + indentWithTab、图标与文案默认值、previewOnlyByDefault 等）原样保留。
 */
...
/**
 * Crepe 自己的 code-mirror feature 配置：kit 的 CodeBlockConfig 之外，还多了
 * `theme` 与两个预览切换按钮的图标/文案回调。
 */
export interface DeskCodeMirrorConfig extends Partial<CodeBlockConfig> {
  theme?: Extension
...
export function codeMirror(editor: Editor, config: DeskCodeMirrorConfig = {}): void {
  editor
    .config((ctx) => {
      const { languages = [], theme } = config
      const extensions = [keymap.of(defaultKeymap.concat(indentWithTab)), basicSetup]
      if (theme) {
      ...
      ctx.update(codeBlockConfig.key, (defaultConfig) => {
        ...
        return {
          ...
          previewToggleButton: (previewOnlyMode) => {
            ...
            const icon =
              ((_a2 = config.previewToggleIcon) == null
                ? void 0
                : _a2.call(config, previewOnlyMode)) ||
              (previewOnlyMode ? editIcon : visibilityOffIcon)
            const text =
              ((_b = config.previewToggleText) == null
                ? void 0
                : _b.call(config, previewOnlyMode)) || (previewOnlyMode ? 'Edit' : 'Hide')
          ...
          previewOnlyByDefault:
            (_a = config.previewOnlyByDefault) != null ? _a : defaultConfig.previewOnlyByDefault
        }
      })
    })
