  ...
  const workspace: TNotesKbWorkspace = {
    ...
    notes: {
      ...
      async create(input) {
        ...
        const { lineIndex, indentLevel, where } = placementLineIndex(
          lines,
          input.placement,
        );
        ...
        const nextLines =
          lines.length === 0
            ? [noteLine]
            : insertLinesRelative(lines, lineIndex, [noteLine], where);
      ...
      async remove(index) {
        ...
        try {
          ...
          const children = collectSubtreeNoteIndexes(lines, lineIndex).filter(
            (i) => i !== index,
          );
    ...
    toc: {
      ...
      async removeEntry(ref) {
        ...
        return {
          ...
          changedFiles: [
            ...toDelete.map(
              (f): ChangedFile => ({ path: f.relPath, kind: "deleted" }),
            ),
            { path: TOC_FILE, kind: "updated" },
          ],
        };
